                ...
                <ul class="px-2 space-y-1">
                    @foreach($projects as $project)
                        <li>
                            <a @class([
                                    'flex items-center h-10 px-2 space-x-2 transition rounded-lg ',
                                    'text-white bg-blue-600' => (int)request()->route('projects') === $project->id,
                                    'hover:bg-gray-500/5 focus:bg-blue-500/10 focus:text-blue-600 focus:outline-none' => (int)request()->route('projects') !== $project->id
                                ])

                               href="{{ route('projects.show', $project->id) }}">
                                <svg
                                    @class(['w-7 h-7', 'text-blue-500' => (int)request()->route('projects') !== $project->id])
