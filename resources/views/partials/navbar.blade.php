            ...
            <ul class="space-y-2 pb-4 border-b border-gray-200 dark:border-white/10">
                <li>
                    <a @class([
                        ...
                        <x-heroicon-o-home @class(['w-5 h-5', 'text-gray-500' => !request()->is('/')]) />
                ...
                <li>
                    <a @class([
                        ...
                        'text-white bg-brand-500 dark:bg-white/5 dark:hover:bg-white/5 dark:text-brand-400' => request()->is(
                            'my'),
                        'hover:bg-gray-500/5 focus:bg-brand-500/10 focus:text-brand-600 focus:outline-none dark:hover:bg-white/5 dark:focus:text-gray-200 dark:text-gray-200' => !request()->is(
                            'my'),
                    ]) href="{{ route('my') }}">
                        <x-heroicon-o-queue-list @class(['w-5 h-5', 'text-gray-500' => !request()->is('my')]) />
                ...
                <li>
                    <a @class([
                        ...
                        <x-heroicon-o-user @class(['w-5 h-5', 'text-gray-500' => !request()->is('profile')]) />
                ...
                <li>
                    <a @class([
                        ...
                        <x-heroicon-o-clock @class(['w-5 h-5', 'text-gray-500' => !request()->is('activity')]) />
                ...
                @if (app(App\Settings\GeneralSettings::class)->enable_changelog)
                    <li>
                        <a @class([
                            ...
                            <x-heroicon-o-rss @class(['w-5 h-5', 'text-gray-500' => !request()->is('changelog*')]) />
                    ...
                    <ul class="space-y-2">
                        @foreach ($projects->groupBy('group') as $group => $groupProjects)
                            ...
                            @foreach ($nonCollapsible as $project)
                                <li>
                                    <a title="{{ $project->title }}" @class([
                                        'flex items-center h-10 px-2 space-x-2 transition rounded-lg',
                                        'text-white bg-brand-500 dark:bg-white/5 dark:hover:bg-white/5 dark:text-brand-400' =>
                                            request()->segment(2) === $project->slug,
                                        'hover:bg-gray-500/5 focus:bg-brand-500/10 focus:text-brand-600 focus:outline-none dark:hover:bg-white/5 dark:focus:text-gray-200 dark:text-gray-200' =>
                                            request()->segment(2) !== $project->slug,
                                    ])
                            ...
                            @if ($collapsible->count() > 0)
                                ...
                                <div x-data="{ open: {{ Request::is('projects/*') ? 'true' : 'false' }} }">
                                    ...
                                    <button class="w-full text-left p-2 rounded hover:bg-gray-700 flex items-center" onclick="toggleDropdown()">
                                        ...
                                        <svg sidebar-toggle-item class="w-6 h-6 transition-transform duration-200" :class="{ 'rotate-180': open }" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <ul id="{{ $groupId }}" x-show="open" x-transition class="py-2 space-y-2">
                                        ...
                                        <div id="dropdownMenu" class="hidden pl-4 py-2 overflow-y-auto ">
                                                ...
                                                <a title="{{ $project->title }}" href="{{ route('projects.show', $project) }}" class="flex items-center block p-2 text-sm">
                                                    <x-dynamic-component :component="$project->icon ?? 'heroicon-o-hashtag'" @class([
                                                        'shrink-0 w-5 h-5',
                                                        'text-gray-500' => request()->segment(2) != $project->slug
                                                        ])/>
 ...
 --}}
                                    ...
                                    <div x-data="{ open: false }" class="space-y-1">
                                        ...
                                        <button @click="open = !open"
                                            class="flex items-center justify-between w-full p-2 rounded-lg hover:bg-gray-700 text-white">
                                            <span>
                                                {{ trans('projects.projects') }}
                                            </span>
                                            <svg :class="{ 'rotate-180': open }" class="w-4 h-4 ms-1.5 -me-0.5 text-white"
